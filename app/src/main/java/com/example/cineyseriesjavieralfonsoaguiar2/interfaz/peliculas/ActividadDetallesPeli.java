...
public class ActividadDetallesPeli extends ActividadBaseMenuLateral {

    @Override
    protected void onCreate(Bundle estadoGuardado) {
        super.onCreate(estadoGuardado);
        setContentView(R.layout.detalles_peli_layout);
        establecerTituloMenuLateral(R.string.detalles);
        ...
        if (extras != null) {
            String uriImagen = extras.getString("uriImagen");
            if (uriImagen != null && !uriImagen.isEmpty()) {
                imgContactDetail.setImageURI(Uri.parse(uriImagen));
            } else {
